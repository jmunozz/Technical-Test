...
const VisualizerColumn = ({ bookings, type, className }) => {
  if (type === 'label') {
    ...
    hours = hours.map((hour, i) => <div key={i} className={`vis-table-body-column-cell vis-table-body-column-cell-header ${(i === 15) ? 'last-bottom' : ''}`} ><span>{`${hour}:00`}</span></div>);
    return (<div className="vis-table-body-column vis-table-body-column-header">{hours}</div>);
  }
  let cells = Array(16).fill(undefined);
  cells = cells.map((v, i) => (
    <div key={i} className={`vis-table-body-column-cell ${(i === 15) ? 'last-bottom' : ''}`}>
  ...
  const slots = bookings.map((booking, i) => <VisualizerSlot key={i} booking={booking} />);
